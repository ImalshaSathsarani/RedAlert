...
const fetchNotifications = async () => {
    try {
      ...
      const res = await axios.get(`http://192.168.154.203:5000/api/notifications/user/${storedUserId}`);
...
const handleResponse = async (notificationId: string, status: 'accepted' | 'declined') => {
  try {
    ...
    await axios.post(`http://192.168.154.203:5000/api/notifications/respond/${notificationId}`, {
      status,
      donorId: userId,
    });
    Alert.alert(
  ...
  <Image
    source={{ uri: `http://192.168.238.203:5000${item.requestId.hospitalId.profilePicture}` }}
